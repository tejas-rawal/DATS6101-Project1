...
The understand the effect of seasonality on the temperature data, we calculated the average daily temperatures by month. As we can see here, there is a trend of rising temperatures when we move from winter to summer months. This is to be expected since summer will have higher average daily maximum and minimum temperatures than winter. Seasonality is an aspect of our data that we might need to account for in our testing, as temperatures ranges clearly differ between summer and winter months.  
...
This distinction was important because we could now compare temperature observations across three independent groups. Each era contains roughly 15,000 daily observations giving us statistically powerful sample sizes to work off of.   
...
The distributions of each era looks similar, however the calculated average daily maximum temperatures for each suggests a slight difference across the time periods. To verify the statistical significance of these differences, we ran an ANOVA test comparing the three eras.
...
```
The plot displays the estimated difference between the means of each era comparison, along with a 95% confidence interval. The adjusted p-value for each 2-way comparison was significantly low which indicates that the differences in the average daily maximum temperature between each era were significant.  

Through this analysis, we are able to safely reject the hypothesis that the average daily maximum temperature remains the same across each era. There is significant evidence to suggest that our TMAX variable is increasing over time. Next, we can utilize linear regressions to dive further into this relationship.  


## Precipitation trends -Tejas' outlier analysis + snowfall at Christmas

As average temperatures on Earth rise, more evaporation occurs which increases overall precipitation. The gauge the effect of this on the local level, we sought to analyze precipitation outliers in our dataset and determine if a significant change has occurred in rainfall over time.  
