...
```
```{r}
linfit_min <- lm(NYmintemp_June$TMIN_avg ~ NYmintemp_June$year)
summary(linfit_min)
...
I wonder if we'll get something different if we look 

I also wonder whether 

##Playing with JFK weather
...
#creating a subset of the weather data that are only date- and temperature-related
JFKweath_temp <- subset(JFKweath, select = c(DATE, day, month, year, TMAX, TMIN, TAVG)) 
xkablesummary(JFKweath_temp)
```